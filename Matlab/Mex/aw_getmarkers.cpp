...
static mxArray *request_markers(const QString& file, const QString& extractTriggers, 
								QVector<int>& values, QStringList& labels, QStringList& channels);


mxArray *request_markers(const QString& file, const QString& extractTriggers, QVector<int>& values, QStringList& labels, QStringList& channels)
{
    ...
    if (nMarkers == 0)
        ...
        for (int i = 0; i < nMarkers; i++) {
            ...
            bool targetChannels;    // flag to determine if channels are targeted by the marker
            float *tmp = NULL;
            qint16 *tmpi = NULL;
            ...
            // value
            mxArray *output_value = mxCreateNumericMatrix(1, 1, mxINT16_CLASS, mxREAL);
            qint16 *v = (qint16 *)mxGetData(output_value);
            v[0] = value;
