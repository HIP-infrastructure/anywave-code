	...
	: QDialog(parent)
{
	...
	sliderCPU->setValue(aws->maxCPUCores);
	// language
...
void AwPrefsDial::accept()
{
	...
	// CPU CORES
	if (sliderCPU->value() != aws->totalCPUCores) {
		aws->maxCPUCores = sliderCPU->value();
		QThreadPool::globalInstance()->setMaxThreadCount(aws->maxCPUCores);
	}
	else
		QThreadPool::globalInstance()->setMaxThreadCount(aws->totalCPUCores);
	QDialog::accept();
}
...
#endif
    if (!folder.isEmpty()) {
		QString previousFolder = lineEditMatlabPath->text();
		//lineEditMatlabPath->setText(dlg.selectedFiles().at(0));
        lineEditMatlabPath->setText(folder);
		if (previousFolder.toUpper() != lineEditMatlabPath->text().toUpper()) {
			// The user changed the folder location => Mac and Linux, redo the detection at start
			QSettings settings;
			settings.setValue("matlab/path", lineEditMatlabPath->text());
			settings.setValue("matlab/detected", true);
			settings.setValue("matlab/user_changed_folder", true);
