	...
	: QWidget(parent)
{
	...
	action = new QAction(tr("Write value to Trigger channel"), this);
	m_menu->addAction(action);
...
void AwMarkerManagerSettings::goToMarkerAtRow(int row)
{
	...
	float pos = (float)tvMarkers->model()->data(index, Qt::EditRole).toDouble();
	emit moveRequest(pos);
}
...
void AwMarkerManagerSettings::goToMarkerPos()
{
	...
	foreach (QModelIndex index, indexes) {
		if (index.column() == MARKER_COLUMN_POS) {
			emit moveRequest((float)tvMarkers->model()->data(index, Qt::EditRole).toDouble());
			break;
		}
	}
}
	...
	if (dlg.exec() == QDialog::Accepted) {
		for (auto i : indexes) {
			if (dlg.replaceValues() && i.column() == MARKER_COLUMN_CODE)
				...
				tvMarkers->model()->setData(i, dlg.label(), Qt::EditRole);
		}
		m_displayedMarkers = m_model->markers();
		emit markersChanged(m_displayedMarkers);
		updateStats();
...
void AwMarkerManagerSettings::launchProcess()
{
	...
	for (auto i : indexes) {
		if (i.column() == 0)
			markers << m_displayedMarkers.at(i.row());
	}
	// Launch Process Dial
...
void AwMarkerManagerSettings::removeMarkers()
{
	...
	if (AwMessageBox::question(this, tr("Remove Markers"), QString(tr("Do you really wand to remove all the selected markers"))) == QMessageBox::No)
		return;
...
void AwMarkerManagerSettings::writeTrigger()
{
	...
	foreach (QModelIndex index, indexes) {
		if (index.column() == 0)
			markers << m_displayedMarkers.at(index.row());
	}
...
void AwMarkerManagerSettings::contextMenuRequested(const QPoint& pos)
{
	...
	if (!index.isValid()) 
		return;
	AwMarker *firstMarker = m_displayedMarkers.at(index.row());
