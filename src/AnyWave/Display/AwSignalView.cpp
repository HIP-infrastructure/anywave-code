...
AwSignalView::AwSignalView(AwViewSettings *settings, int flags, QObject *parent) :
	AwBaseSignalView(flags, settings, parent)
{
	...
	AwMarkersBar *markBar = new AwMarkersBar(m_physics, widget());
...
void AwSignalView::updatePositionInFile(float pos)
{
	if (pos != m_positionInFile) {
		...
		requestData(&m_channels, pos, m_pageDuration);
		dataReceived();
	}
}
...
void AwSignalView::synchronizeOnPosition(float pos)
{
	...
	requestData(&m_channels, newPos, m_pageDuration);
	dataReceived();
}
...
void AwSignalView::applyChannelFilters()
{
	...
	//// destroying current montage (must contain only REAL channels)
	//while (!m_channels.isEmpty())
	//	delete m_channels.takeFirst();
	...
	//// re adding virtual channels if matches filters
	//foreach (AwChannel *c, m_virtualChannels)	{
	//	if (m_settings->filters.contains(c->type()))
	//		m_channels << c;
	//}
...
void AwSignalView::setChannels(AwChannelList& channels)
{
	...
	applyChannelFilters();
	m_scene->setChannels(m_channels);
	requestData(&m_channels, m_positionInFile, m_scene->pageDuration());
	dataReceived();
}
