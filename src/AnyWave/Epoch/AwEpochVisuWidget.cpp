	...
	: QWidget(parent)
{
	...
	updateConditions();
	m_signalView = new AwBaseSignalView(AwBaseSignalView::NoNavButtons | AwBaseSignalView::NoInfoLabels | AwBaseSignalView::ViewAllChannels
		| AwBaseSignalView::NoHScrollBar | AwBaseSignalView::NoMarkerBar);
	m_ui.groupBoxEpoch->layout()->replaceWidget(m_ui.widget, m_signalView->widget());
	m_zeroMarker = NULL;
	m_buffer = new AwDataBuffer;
	m_buffer->openConnection(m_signalView);
