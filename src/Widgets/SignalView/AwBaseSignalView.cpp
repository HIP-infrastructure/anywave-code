...
AwBaseSignalView::AwBaseSignalView(int flags, AwViewSettings *settings, QObject *parent)
	: AwDataClient(parent) 
{
	m_flags = flags;
	if (settings == NULL)
	...
	m_navBar = new AwNavigationBar(m_widget, flags);
	m_markerBar = new AwBaseMarkerBar(m_physics, m_widget);
...
void AwBaseSignalView::changeObjects(AwGraphicsView *v, AwGraphicsScene *s, AwNavigationBar *navBar, AwBaseMarkerBar *markBar)
{
	...
	if (v) {
		m_widget->layout()->replaceWidget(m_view, v);
		delete m_view;
		m_view = v;
	}
	if (s) {
	...
	if (navBar) {
		m_widget->layout()->replaceWidget(m_navBar, navBar);
		delete m_navBar;
		m_navBar = navBar;
	}
	if (markBar) {
		m_widget->layout()->replaceWidget(m_markerBar, markBar);
		delete m_markerBar;
		m_markerBar = markBar;
	}
	// redo connections
...
void AwBaseSignalView::setChannels(AwChannelList& channels)
{
