...
AwMapWidget::AwMapWidget(QWidget *parent, int flags) : QVTKWidget(parent)
{
#if VTK_MAJOR_VERSION > 5
...
//	m_black[0] = m_black[1] = m_black[2] = 0.0;
	m_red[0] = 1.0;
	m_red[1] = m_red[2] = 0.0;
...
#if QT_VERSION < 5
		m_latencyActor->SetInput(sLatency.toAscii().data());
#else
...
#if QT_VERSION < 5
		m_latencyActor->SetInput(sInterval.toAscii().data());
		m_textFlagActor->SetInput(m_textFlag.toAscii().data());
#else
