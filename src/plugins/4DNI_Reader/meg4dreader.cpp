...
NI4DReader::NI4DReader() : AwFileIOPlugin()
{
  ...
  description = QString(tr("Reads 4DNI Meg files"));
  manufacturer = "4DNI";
  version = QString("1.0");
  fileExtensions << "*,*";
...
#pragma omp for
					for (qint64 i = 0; i < c->dataSize(); i++) {
						// copy to channel
						float val = buffer[index + i * nChannelsTotal];
...
int NI4DFileReader::clearTriggerChannel(const QString& name)
{
	if (m_triggers.isEmpty())
		return -1;

	int index =	infos.indexOfChannel(name);	// index of channel trigger in data file.
	if (index == -1)
		return -1;
	...
	if (!fileWrite.open(QIODevice::ReadWrite))
		return -1;
	float sampleRate = infos.channels().at(index)->samplingRate();
	...
	qint64 sample = 0;
	qint64 dataPos = index * m_dataSize;

	// start position of trigger channel
	while (dataPos < m_headerPos) { // dot not write data over header position.
		fileWrite.seek(dataPos);
		switch (m_dataFormat)
		{
		case Float:
			stream << (float)0;
...
int NI4DFileReader::writeTriggerChannel(const QString& channelName, const AwMarkerList& list)
{
	...
	if (!fileWrite.open(QIODevice::ReadWrite))	{
		emit triggerValuesWritten(false, 0);
		return -1;
	}
	...
	for (auto m : list)	{
		...
		quint32 sample = (quint32)floor((m->start() * sampleRate));
		quint32 startSample = sample * nbChannels;
		float value = m->value();
		qint16 shortValue = (qint16)floor(value);
		qint32 intValue = (qint32)floor(value);
		fileWrite.seek(startSample * m_dataSize + index * m_dataSize);

		for (quint32 i = 0; i < sampleDuration; i++) {
			...
			case Short:
				{
					short valToStore = (short)floor(shortValue * channel_data->gain / (channel_data->units_per_bit * channel_data->scale));
					stream << valToStore;
				}
				break;
			case Long:
				{
					qint32 valToStore = (qint32)floor(intValue * channel_data->gain / (channel_data->units_per_bit * channel_data->scale));
					stream << valToStore;
				}
				break;
			}

			fileWrite.seek((startSample * m_dataSize) + (index * m_dataSize) + ((i + 1) * nbChannels * m_dataSize));
		}
	}
	fileWrite.close();
	emit triggerValuesWritten(true, list.size());
	return 0;
}
