...
#define FICA_TOL 1e-7

using namespace FICAAlgo;
...
int pcamat(const mat vectors, const int numOfIC, int firstEig, int lastEig, mat & Es, vec & Ds);
...
double *FICAAlgo::getW()
{
	return dataW;
}

bool FICAAlgo::FICA(const AwChannelList& channels, int modality, int m, int n, int nc)
{
	mixedSig = mat(m, n);
	for (int i = 0; i < m; i++) {
		AwChannel *c = channels.at(i);
		for (int j = 0; j < n; j++) {
			mixedSig.set(i, j, (double)c->data()[j]);
		}
	}

	if (modality == AwChannel::MEG) // resizing meg data
		mixedSig = mixedSig * 1E12;
	...
	// Init default values
	 ...
	 if (separate()) {
		 dataA = new double[A.rows() * A.cols()];
		 double *save = dataA;
		 for (int i = 0; i < A.rows(); i++)
			 for (int j = 0; j < A.cols(); j++)
				 *dataA++ = A.get(i, j);
		 dataA = save;
		 dataW = new double[W.rows() * W.cols()];
		 save = dataW;
