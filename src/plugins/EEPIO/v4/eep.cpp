...
#define LIBEEP_VERSION_PATCH 178
...
_libeep_allocate() {
  ...
  new_entry_map = (struct _libeep_entry **)realloc(_libeep_entry_map, sizeof(struct _libeep_entry *) * (_libeep_entry_size + 1));
...
_libeep_recinfo_allocate() {
  ...
  new_recinfo_map = (struct record_info_s **)realloc(_libeep_recinfo_map, sizeof(struct record_info_s *) * (_libeep_recinfo_size + 1));
...
_libeep_channels_allocate() {
  ...
  new_channel_map = (struct _libeep_channels**) realloc(_libeep_channel_map, sizeof(struct _libeep_channels *) * (_libeep_channel_size + 1));
