...
int read_chanseq_chunk(eeg_t *EEG, storage_t *store, uint64_t expected_length)
{
  ...
  {
	// Modified B Colombet to compile with a C++ compiler
	int _chanin = (int)chanin;
    read_s16(EEG->f, &_chanin);
    store->chanseq[compchan] = (short)_chanin;
	// Modified B Colombet to compile with a C++ compiler
  }
  return CNTERR_NONE;
...
int eep_get_samplec_full(const eeg_t *cnt, uint64_t *samplec)
{
	...
	// Modified B Colombet to apply C++ compiler rules
	// original C code:
		//store = &cnt->store[cnt->current_datachunk];
		//(*samplec) = store->data.writepos + cnt->eep_header.samplec;
	(*samplec) = cnt->store[cnt->current_datachunk].data.writepos + cnt->eep_header.samplec;
	// Modified B Colombet to apply C++ compiler rules
	return CNTERR_NONE;
}
