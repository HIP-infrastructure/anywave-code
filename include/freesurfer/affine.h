...
typedef struct alignas(16) _av {
 // float vec[kAffineVectorSize] __attribute__ ((aligned (16)));
    float vec[kAffineVectorSize];
} AffineVector;
...
void AffineMatrixAlloc( AffineMatrix **am ) {
    // aligned_alloc is not implemented in Microsoft Visual C++
#ifdef _MSC_VER
    
#else
 // void* tmp = NULL;     // gcc complains otherwise
  void* tmp = std::aligned_alloc(16, sizeof(AffineMatrix));
