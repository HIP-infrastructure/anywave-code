...
assert(numel(markers) == 5, 'expecting 5 markers. failed.');
expected_labels = { 'm1', 'm2', 'm3', 'm4', 'm5' };
labels = { markers(:).label };
assert(size(labels, 2) == size(expected_labels, 2), 'labels number mismatch. failed.');
assert(all(ismember(labels, expected_labels)) == true, 'labels mismatch. failed.');
expected_values = single([ -1, 1, 5.5, 10, 22]);
values = [ markers(:).value ];
assert(size(expected_values, 2) == size(values, 2) , 'values number mismatch. failed.');
assert(all(ismember(expected_values, values)) == true, 'values mismatch. failed.');
% check for durations of each markers
expected_durations = single([0, 8, 0, 2, 0]);
durations = [ markers(:).duration ];
assert(all(ismember(expected_durations, durations)) == true, 'duration values mismatch. failed.');
expected_targets = { 'A2'};
% m5 must target A2
assert(size(markers(5).channels, 2) == size(expected_targets, 2), 'm5 must target channel A2. failed.');
assert(all(cellfun(@isequal, markers(5).channels, expected_targets)) == true, 'm5 must target channel A2. failed.');
% ask for a specific marker label
cfg = [];
cfg.labels = { 'm1', 'm5' };
markers = aw_getmarkers_ex(cfg);
result = { markers(:).label };
assert(size(result, 2) == size(cfg.labels, 2), 'requested m1 and m5, got wrong result. failed.');
assert(all(cellfun(@isequal, cfg.labels, result)) == true, 'requested m1 and m5, got wrong result. failed.');
